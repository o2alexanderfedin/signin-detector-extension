...
**Confidence:** HIGH (build tooling, testing, types) / MEDIUM (PSL, JWT-shape libraries) — see per-row notes
...
npx wxt@latest init sign-in-detector -t vanilla-ts
cd sign-in-detector
...
# Testing
npm install -D vitest @webext-core/fake-browser @playwright/test jsdom

# Types
npm install -D @types/chrome

# Lint (ESLint 9/10 flat config + type-aware TS rules)
npm install -D eslint typescript-eslint @vitest/eslint-plugin
```
...
**Given this project targets Chrome MV3 + Firefox-compatible from one codebase:**
...
**Given the TDD-heavy process constraint (project explicitly requires TDD):**
...
---
*Stack research for: Chrome MV3 (Firefox-compatible) sign-in detector browser extension*
*Researched: 2026-07-17*
